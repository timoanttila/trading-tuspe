...
My main indicator, [Trend is your friend](./FollowTrend.pine), combines four different moving averages with displaced and inside bar detection, pullback recognition, PDH and PHL levels and some risk management rules to form a complete trading system.
...
| --- | --- | --- | --- | --- |
| `ma_short` | 9 | EMA | Aqua | Crossover / Soft exit |
| `ma_med` | 20 | SMA | Orange | Crossover / Hard exit |
| `ma_base` | 50 | SMA | Yellow | Base for trade entries and filtering |
| `ma_long` | 200 | SMA | White | Strong support or resistance |
...
Buy or sell signals happen when `ma_short` crosses over or under `ma_med` and both moving averages are above `ma_base`. There can also be early entries when `ma_short` crosses `ma_med` but `ma_base` is still against the trade. For a valid signal, the candle must have a body that's at least 40% of its total size and must close above or below the last four candles, depending on the direction. This helps reduce false signals.
...
A gray `xcross` signals a possible exit. This happens when the price crosses `ma_short` in the opposite direction.
...
The key to long-term success is solid risk management. Cut losses quickly and let winners run as long as the price stays above `ma_short` or `ma_med`. Using a trailing stop loss can help lock in profits as the trade moves in your favor.
...
Avoid trading against the direction of `ma_base` and `ma_long`. If those moving averages are close to your entry, it's better to wait for a clear price reaction. The best entries usually come after a breakout followed by a successful retest.
